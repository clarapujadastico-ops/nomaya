...
@class NSURL;
@class NSLocale;
enum HCaptchaSize : NSInteger;
enum HCaptchaOrientation : NSInteger;
@class UIView;
@class HCaptchaResult;
/// hCaptcha SDK facade (entry point)
SWIFT_CLASS("_TtC14StripePayments8HCaptcha")
@interface HCaptcha : NSObject
/// Initializes a HCaptcha object
/// Both <code>apiKey</code> and <code>baseURL</code> may be nil, in which case the lib will look for entries of <code>HCaptchaKey</code> and
/// <code>HCaptchaDomain</code>, respectively, in the project’s Info.plist
/// \param apiKey The API key sent to the HCaptcha init
///
/// \param baseURL The base URL sent to the HCaptcha init
///
/// \param locale A locale value to translate HCaptcha into a different language
///
/// \param size A HCaptcha size check <code>HCaptchaSize</code> for more details
///
/// \param orientation A HCaptcha orientation: <code>.portrait</code> or <code>.landscape</code> is available
///
/// \param jsSrc See Enterprise docs
///
/// \param rqdata See Enterprise docs.
///
/// \param sentry See Enterprise docs
///
/// \param endpoint See Enterprise docs
///
/// \param reportapi See Enterprise docs
///
/// \param assethost See Enterprise docs
///
/// \param imghost See Enterprise docs
///
/// \param host See Enterprise docs
///
/// \param theme HCaptcha supports <code>.light</code>, <code>dark</code> and <code>.contrast</code> themes
///
/// \param customTheme See Enterprise docs
///
/// \param diagnosticLog Emit detailed console logs for debugging
///
///
/// throws:
/// Rethrows any exceptions thrown by <code>String(contentsOfFile:)</code>
- (nullable instancetype)initWithApiKey:(NSString * _Nullable)apiKey passiveApiKey:(BOOL)passiveApiKey baseURL:(NSURL * _Nullable)baseURL locale:(NSLocale * _Nullable)locale size:(enum HCaptchaSize)size orientation:(enum HCaptchaOrientation)orientation jsSrc:(NSURL * _Nonnull)jsSrc rqdata:(NSString * _Nullable)rqdata sentry:(BOOL)sentry endpoint:(NSURL * _Nullable)endpoint reportapi:(NSURL * _Nullable)reportapi assethost:(NSURL * _Nullable)assethost imghost:(NSURL * _Nullable)imghost host:(NSString * _Nullable)host theme:(NSString * _Nonnull)theme customTheme:(NSString * _Nullable)customTheme diagnosticLog:(BOOL)diagnosticLog error:(NSError * _Nullable * _Nullable)error;
/// Starts the challenge validation
/// \param view The view that should present the webview.
///
/// \param resetOnError If HCaptcha should be reset if it errors. Defaults to <code>true</code>.
///
/// \param completion A closure that receives a HCaptchaResult which may contain a valid result token.
///
- (void)validateOn:(UIView * _Nullable)view resetOnError:(BOOL)resetOnError completion:(void (^ _Nonnull)(HCaptchaResult * _Nonnull))completion;
...
typedef SWIFT_ENUM(NSInteger, HCaptchaOrientation, closed) {
  HCaptchaOrientationPortrait = 0,
  HCaptchaOrientationLandscape = 1,
};

/// The HCaptcha result.
/// This may contain the validation token on success, or an error that may have occurred.
SWIFT_CLASS("_TtC14StripePayments14HCaptchaResult")
@interface HCaptchaResult : NSObject
/// Tries to unwrap the Result and retrieve the token if it’s successful.
///
/// throws:
/// <code>HCaptchaError</code>
///
/// returns:
/// The validation token uppon success.
- (NSString * _Nullable)dematerializeAndReturnError:(NSError * _Nullable * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

/// Widget display mode
typedef SWIFT_ENUM(NSInteger, HCaptchaSize, closed) {
  HCaptchaSizeInvisible = 0,
  HCaptchaSizeCompact = 1,
  HCaptchaSizeNormal = 2,
};
...
/// If this is a PayPay PaymentMethod (i.e. <code>self.type == STPPaymentMethodTypePayPay</code>), this contains additional details.
@property (nonatomic, readonly, strong) STPPaymentMethodPayPay * _Nullable payPay;
...
@class STPPaymentMethodPayPayParams;
...
@property (nonatomic, strong) STPPaymentMethodPayPayParams * _Nullable payPay;
/// Radar options that may contain HCaptcha token
@property (nonatomic, strong) STPRadarOptions * _Nullable radarOptions;
...
/// \param payPay An object containing additional PayPay details.
///
/// \param metadata Additional information to attach to the PaymentMethod.
///
- (nonnull instancetype)initWithPayPay:(STPPaymentMethodPayPayParams * _Nonnull)payPay metadata:(NSDictionary<NSString *, NSString *> * _Nullable)metadata;
+ (NSString * _Nullable)rootObjectName SWIFT_WARN_UNUSED_RESULT;
+ (NSDictionary<NSString *, NSString *> * _Nonnull)propertyNamesToFormFieldNamesMapping SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// seealso:
/// https://stripe.com/docs/payments/paypay
SWIFT_CLASS("_TtC14StripePayments22STPPaymentMethodPayPay")
@interface STPPaymentMethodPayPay : NSObject <STPAPIResponseDecodable>
@property (nonatomic, readonly, copy) NSDictionary * _Nonnull allResponseFields;
/// :nodoc:
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
+ (nullable instancetype)decodedObjectFromAPIResponse:(NSDictionary * _Nullable)response SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS("_TtC14StripePayments28STPPaymentMethodPayPayParams")
@interface STPPaymentMethodPayPayParams : NSObject <STPFormEncodable>
@property (nonatomic, copy) NSDictionary * _Nonnull additionalAPIParameters;
+ (NSString * _Nullable)rootObjectName SWIFT_WARN_UNUSED_RESULT;
+ (NSDictionary<NSString *, NSString *> * _Nonnull)propertyNamesToFormFieldNamesMapping SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
